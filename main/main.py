        ...
        for epoch in range(start_epoch, total_epoch):
            ...
            try:
                for batch, data in enumerate(tdataset):
                    ...
                    tf.summary.scalar('train_loss', loss.numpy(), step=epoch*1250*5+batch)      # the tdataset has been repeated 5 times..
                    tf.summary.text('Zernike_coe_pred', tf.as_string(tf.squeeze(pred)), step=epoch*1250*5+batch)
                    tf.summary.text('Zernike_coe_gt', tf.as_string(tf.squeeze(labels)), step=epoch*1250*5+batch)
